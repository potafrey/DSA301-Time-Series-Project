...
obama_seasonal_monthly <- obama_mstl[,5]
```
...
#ARIMA Model
```{r}
nsdiffs(obama_s_adj) #returns 0
ndiffs(obama_s_adj) #returns 1

obama_arima <- auto.arima(diff(obama_s_adj))
#ARIMA(5,0,1) was returned

obama_arima_fc <- forecast(obama_arima, h = 24)
autoplot(obama_arima_fc)
...
dem_ts_daily <- ts(data = dem_pageviews_daily$views, start = c(2016,01), frequency = 365)

obama_arimax <- auto.arima(diff(obama_s_adj), xreg = dem_pageviews_daily)
